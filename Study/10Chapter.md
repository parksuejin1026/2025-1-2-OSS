...
Git은 세 가지 영역(작업 디렉토리, 스테이징 영역, 깃 저장소) 간의 파일을 관리하며, `reset`과 `revert`는 이 영역에 영향을 줍니다.

| 영역 | 역할 | `reset` `--hard` 시 영향 |
| :--- | :--- | :--- |
| **작업 디렉토리 (Working Directory)** | 실제 파일을 수정하는 공간 | 변경 사항 삭제 후 커밋 내용으로 복사 |
| **스테이징 영역 (Staging Area)** | 다음 커밋을 준비하는 공간 | 변경 사항 삭제 후 커밋 내용으로 복사 |
| **깃 저장소 (Git Repository)** | 커밋 이력(버전)이 저장되는 곳 | HEAD 포인터가 이전 커밋으로 이동하며 이력 삭제 |

---

## 2. 버전 되돌리기 명령: `git reset` (타임 머신)

**`git reset`**은 커밋 이력에서 **특정 커밋으로 완전히 되돌아가는** 기능입니다. 이동된 커밋 이후의 이력은 **모두 사라집니다**.
...
* **되돌리기 취소 (복원):** `git reset --hard ORIG_HEAD` 명령을 사용하면 직전 `reset` 실행 이전 상태로 복원할 수 있습니다.
